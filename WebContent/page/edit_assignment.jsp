			...
			<div class="layui-form-item">
			    <div class="layui-inline" style="margin-top: 20px;">
			      ...
			      <div class="layui-input-inline grade" >
			      	<input type="text" style="width: 400px" name="assignmentName" id="assignmentName" autocomplete="off" class="layui-input">
			      	<input type="hidden" style="width: 400px" name="assignmentId" id="assignmentId" autocomplete="off" class="layui-input">
			      </div>
			    </div>
			</div>
			<div class="layui-form-item">
...
layui.use(['table','form', 'laydate'], function(){
	...
	form.on('submit(updateAssignment)', function(data){
		...
		$.ajax({
			url:'<%=basePath%>employe/updateAssignment',
			type:'post',
			dataType:'json',
...
function loadAssignment(assignment){
	$("select[name='gradeId']").find("option[value='" + assignment.gradeId + "']").prop("selected",true);
	$("select[name='courseId']").find("option[value='" + assignment.courseId + "']").prop("selected",true);
	$("input[name='assignmentName']").val(assignment.assignmentName);
	$("input[name='assignmentId']").val(assignment.assignmentId);
	$("input[name='fileUrl']").val(assignment.fileUrl);
	form.render();
}
