...
<body>
			...
			<div class="sidebar-scroll">
				<nav>
					<ul class="nav">
						...
						<li><a href="#" type="employe" class="active" id="assignment"><i class="fa fa-file-text-o"></i> <span>作业列表</span></a></li>
		...
		<div class="main">
			...
			<div class="main-content">
				<div class="container-fluid">
					<div>
						<button type="button" class="layui-btn" lay-submit lay-filter="addAssignment" id="addAssignment">新增</button>
					</div>
					<div>
	...
	<script type="text/html" id="opertion">
		...
		<button style="width: 70px;" id="deleteProduct" class="layui-btn layui-btn-xs" lay-event="assignmentList"><i class="icons icon_revise"></i>提交列表</button>
	</script>
	<script type="text/javascript">
		...
		$("#addAssignment").click(function(){
			layer.open({
				...
				title : '新增',
				btnAlign: 'c',
			  	content: '<%=basePath%>page/edit_assignment.jsp',
			  	btn : ['确定','取消'],
			  	area : ['600px','460px'],
			  	yes : function(index, layero){
			  		var body = layer.getChildFrame('body',index);//建立父子联系
		            var iframeWin = window[layero.find('iframe')[0]['name']];
		           	iframeWin.updateAssignment();
			  	}
			});
		});
					...
					layer.open({
						...
						btnAlign: 'c',
					  	...
					  	success:function(layero, index){
					  		var body = layer.getChildFrame('body',index);//建立父子联系
				            var iframeWin = window[layero.find('iframe')[0]['name']];
				           	iframeWin.loadAssignment(obj.data);
					  	}
					});
				}else if(obj.event === 'assignmentList'){
					layer.open({
						type : 2,				
						title : '编辑',
						btnAlign: 'c',
					  	content: '<%=basePath%>page/assignment_detail.jsp',
					  	btn : ['确定','取消'],
					  	area : ['1000px','620px'],
					  	success:function(layero, index){
					  		var body = layer.getChildFrame('body',index);//建立父子联系
		...
		function loadTable(){
			layui.use(['laydate','form','table'], function(){
				...
				table.render({
					elem: '#assignments'
				    ,height: 490
				    ,url: '<%=basePath%>employe/getAssignmentList' //数据接口
				    ,page: true //开启分页
				    ,cols: [[ //表头
