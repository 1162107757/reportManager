...
<body>
			...
			<div class="sidebar-scroll">
				<nav>
					<ul class="nav">
						...
						<li><a href="#" type="student" class="active" id="assignment"><i class="fa fa-file-text-o"></i> <span>作业列表</span></a></li>
	...
	<script type="text/html" id="opertion">
		...
		<button style="width: 70px;" id="deleteProduct" class="layui-btn layui-btn-xs" lay-event="uploadAssignment"><i class="icons icon_revise"></i>提交作业</button>
	</script>
	<script type="text/javascript">
		...
		layui.use('table', function(){
			...
			table.on('tool(assignments)', function(obj){
				...
				}else if(obj.event === 'uploadAssignment'){
					layer.open({
						type : 2,				
						title : '提交作业',
						btnAlign: 'c',
					  	content: '<%=basePath%>page/uploadAssignment.jsp',
					  	btn : ['确定','取消'],
					  	area : ['600px','230px'],
					  	yes : function(index, layero){
					  		var body = layer.getChildFrame('body',index);//建立父子联系
		...
		function loadTable(){
			layui.use(['laydate','form','table'], function(){
				...
				table.render({
					elem: '#assignments'
				    ,height: 490
				    ,url: '<%=basePath%>student/getAssignmentList' //数据接口
				    ,page: true //开启分页
				    ,cols: [[ //表头
