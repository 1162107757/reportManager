	...
	<div id="wrapper">
		...
		<div id="sidebar-nav" class="sidebar">
			<div class="sidebar-scroll">
				<nav>
					<ul class="nav">
						<li><a href="#" type="admin" class="active" id="employeManager"><i class="fa fa-user"></i> <span>教师管理</span></a></li>
		...
		<div class="main">
			...
			<div class="main-content">
				<div class="container-fluid">
					<div>
						<button type="button" class="layui-btn" lay-submit lay-filter="addEmploye" id="addEmploye">新增</button>
					</div>
					<div>
						<table id="employes" lay-filter="employes"></table>
					</div>
	...
	<script type="text/javascript">
		$("#addEmploye").click(function(){
			layer.open({
				...
				title : '新增',
				btnAlign: 'c',
			  	content: '<%=basePath%>page/edit_employe.jsp',
			  	btn : ['确定','取消'],
			  	area : ['400px','240px'],
			  	yes : function(index, layero){
			  		var body = layer.getChildFrame('body',index);//建立父子联系
		            var iframeWin = window[layero.find('iframe')[0]['name']];
		           	iframeWin.updateEmploye();
			  	}
			});
		});
		layui.use('table', function(){
			var table = layui.table;
			table.on('tool(employes)', function(obj){
				if(obj.event === 'edit'){
					layer.open({
						type : 2,				
						title : '编辑',
						btnAlign: 'c',
					  	...
					  	success:function(layero, index){
					  		var body = layer.getChildFrame('body',index);//建立父子联系
				            var iframeWin = window[layero.find('iframe')[0]['name']];
				           	iframeWin.loadEmploye(obj.data);
					  	}
					});
				}
			})
		...
		function loadTable(){
			layui.use(['laydate','form','table'], function(){
				...
				table.render({
					elem: '#employes'
				    ,height: 490
				    ,url: '<%=basePath%>admin/getEmployeList' //数据接口
				    ,page: true //开启分页
				    ,cols: [[ //表头
				    	{type: 'numbers', title: '序号'}
						,{field: 'employeName', title: '教师名字'} 
						,{field: 'workNumber', title: '教师工号'} 
