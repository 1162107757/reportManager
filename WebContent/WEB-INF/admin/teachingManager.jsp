			...
			<div class="sidebar-scroll">
				<nav>
					<ul class="nav">
						...
						<li><a href="#" type="admin" class="" id="gradeManager"><i class="fa fa-tag"></i> <span>班级管理</span></a></li>
						<li><a href="#" type="admin" class="" id="courseManager"><i class="fa fa-tasks"></i> <span>课程管理</span></a></li>
						<li><a href="#" type="admin" class="active" id="teachingManager"><i class="fa fa-tasks"></i> <span>授课列表</span></a></li>
					</ul>
				</nav>
			</div>
		</div>
		...
		<div class="main">
			...
			<div class="main-content">
				<div class="container-fluid">
					<div>
						<button type="button" class="layui-btn" lay-submit lay-filter="addTeaching" id="addTeaching">新增</button>
					</div>
					<div>
						<table id="teachings" lay-filter="teachings"></table>
					</div>
	...
	<script type="text/javascript">
		...
		$("#addTeaching").click(function(){
			layer.open({
				...
				title : '新增',
				btnAlign: 'c',
			  	content: '<%=basePath%>page/edit_teaching.jsp',
			  	btn : ['确定','取消'],
			  	area : ['600px','500px'],
			  	yes : function(index, layero){
			  		var body = layer.getChildFrame('body',index);//建立父子联系
		            var iframeWin = window[layero.find('iframe')[0]['name']];
		           	iframeWin.updateTeaching();
			  	}
			});
		});
		...
		layui.use('table', function(){
			...
			table.on('tool(teachings)', function(obj){
				if(obj.event === 'edit'){
					layer.open({
						type : 2,				
						title : '编辑',
						btnAlign: 'c',
					  	...
					  	success:function(layero, index){
					  		var body = layer.getChildFrame('body',index);//建立父子联系
				            var iframeWin = window[layero.find('iframe')[0]['name']];
				           	iframeWin.loadTeaching(obj.data);
					  	}
					});
				}
			})
		...
		function loadTable(){
			layui.use(['laydate','form','table'], function(){
				...
				table.render({
					elem: '#teachings'
				    ,height: 490
				    ,url: '<%=basePath%>admin/getTeachingList' //数据接口
				    ,page: true //开启分页
				    ,cols: [[ //表头
				    	{type: 'numbers', title: '序号'}
						,{field: 'employeName', title: '教师名称'} 
						,{field: 'gradeName', title: '班级名称'} 
						,{field: 'courseName', title: '课程名称'} 
						,{field: 'classTime', title: '上课时间',templet:function(d){
							return formatDateTwo(d.classTime);
						}} 
